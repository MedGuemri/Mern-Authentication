        ...
        <div className="p-8">
            ...
            <form onSubmit={handlsingup}>
               ...
               {error && <p className='text-red-500 text-sm font-semibold mt-2'>{error}</p>}
                <PasswordStrenghtMeter  password={formData.password}/>
                ...
                <motion.button className={`mt-5 w-full py-3 px-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white 
						...
						hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2
						 focus:ring-offset-gray-900 transition duration-200 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                whileHover={{scale:1.02}}
                whileTap={{scale:0.98}}
                type="submit"
                disabled={isLoading}
                >
                 {isLoading? <Loader className="animate-spin mx-auto " size={24}/> : "Sing up" }   
              </motion.button>
            </form>
